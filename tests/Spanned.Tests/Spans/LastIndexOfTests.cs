...
public class LastIndexOfTests
{
    ...
    public void LastIndexOf_Generic<T>(T[] sourceArray, T missingValue)
    {
        ...
        for (int i = 0; i < source.Length; i++)
        {
            ...
            int foundValueIndex = source.LastIndexOf(searchValue, comparer: null);
            Assert.True(foundValueIndex >= 0 && foundValueIndex < source.Length);
            Assert.StrictEqual(searchValue, source[foundValueIndex]);

            foundValueIndex = source.LastIndexOf(searchValue, comparer: EqualityComparer<T>.Default);
            Assert.True(foundValueIndex >= 0 && foundValueIndex < source.Length);
            Assert.StrictEqual(searchValue, source[foundValueIndex]);
        }
        Assert.StrictEqual(-1, source.LastIndexOf(missingValue, comparer: null));
        Assert.StrictEqual(-1, source.LastIndexOf(missingValue, EqualityComparer<T>.Default));
        ...
        for (int i = 0; i < readOnlySource.Length; i++)
        {
            ...
            int foundValueIndex = readOnlySource.LastIndexOf(searchValue, comparer: null);
            ...
            foundValueIndex = readOnlySource.LastIndexOf(searchValue, comparer: EqualityComparer<T>.Default);
            Assert.True(foundValueIndex >= 0 && foundValueIndex < readOnlySource.Length);
            Assert.StrictEqual(searchValue, readOnlySource[foundValueIndex]);
        }
        Assert.StrictEqual(-1, readOnlySource.LastIndexOf(missingValue, comparer: null));
        Assert.StrictEqual(-1, readOnlySource.LastIndexOf(missingValue, EqualityComparer<T>.Default));
    }

    public static IEnumerable<object?[]> LastIndexOf_Generic_CustomComparer_TestData()
    {
        ...
        yield return new object?[] { new string[] { "Aardvark", "Zyzzyva", "zebra", "Antelope", "Zebra" }, "zebra", StringComparer.InvariantCultureIgnoreCase, 4 };
        yield return new object?[] { new string[] { "Aardvark", "Zyzzyva", "Zebra", "Antelope", "zebra" }, "Zebra", StringComparer.InvariantCultureIgnoreCase, 4 };
    }

    [Theory]
    [MemberData(nameof(LastIndexOf_Generic_CustomComparer_TestData))]
    public void LastIndexOf_Generic_CustomComparer<T>(T[] sourceArray, T value, IEqualityComparer<T>? comparer, int expectedOutput)
    {
        Span<T> source = sourceArray;
        Assert.Equal(expectedOutput, source.LastIndexOf(value, comparer));

        // ---------------------------------

        ReadOnlySpan<T> readOnlySource = sourceArray;
        Assert.Equal(expectedOutput, readOnlySource.LastIndexOf(value, comparer));
    }

    public static IEnumerable<object[]> LastIndexOf_Generic_EmptySource_ReturnsMinusOne_TestData()
    {
    ...
    [Theory]
    [MemberData(nameof(LastIndexOf_Generic_EmptySource_ReturnsMinusOne_TestData))]
    public void LastIndexOf_Generic_EmptySource_ReturnsMinusOne<T>(T[] value)
    {
        Span<T> source = Span<T>.Empty;
        Assert.StrictEqual(-1, source.LastIndexOf(value[0], comparer: null));
        Assert.StrictEqual(-1, source.LastIndexOf(value[0], EqualityComparer<T?>.Default));

        // ---------------------------------

        ReadOnlySpan<T> readOnlySource = ReadOnlySpan<T>.Empty;
        Assert.StrictEqual(-1, readOnlySource.LastIndexOf(value[0], comparer: null));
        Assert.StrictEqual(-1, readOnlySource.LastIndexOf(value[0], EqualityComparer<T>.Default));
    }
}
